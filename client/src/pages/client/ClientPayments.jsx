...
import { FileText, CheckCircle2, X, Printer, Phone, Mail } from 'lucide-react';
              ...
              {!profile.paymentHistory || profile.paymentHistory.length === 0 ? (
                ...
                profile.paymentHistory.map((pmt) => (
                  <tr key={pmt._id || pmt.paymentId} className="hover:bg-surface-divider/80 transition-all group">
                    ...
                    <td className="p-5 text-center">
                      <span className={`px-2 py-1 rounded text-[10px] font-black uppercase tracking-widest ${pmt.paymentMethod === 'cash' ? 'text-emerald-400 bg-emerald-400/5' : 'text-blue-400 bg-blue-400/5'} mx-auto block w-fit`}>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-xs pb-2">
                <div>
                  ...
                  <p className="font-black text-gray-900 text-sm">{profile.personalInfo?.name}</p>
                  <p className="text-text-secondary mt-0.5">Client ID: {profile.clientId || 'N/A'}</p>
                  <p className="text-text-muted mt-0.5">Member since: {profile.createdAt ? new Date(profile.createdAt).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }) : 'Jan 2024'}</p>
                </div>
                <div className="text-right">
              ...
              <div className="pt-3 border-t border-gray-200 text-center space-y-2">
                ...
                <div className="flex justify-center items-center gap-5 text-[10px] text-gray-600 font-bold">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span>{gymInfo?.gymEmail || "support@gym.com"}</span>
                  </div>
                </div>
