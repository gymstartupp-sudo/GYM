...
import { LayoutDashboard, Users, CreditCard, ClipboardList, AlertCircle, UserMinus, Clock, UserPlus, MessageSquare, Trash2 } from 'lucide-react';
...
const Sidebar = () => {
  ...
  const navLinks = [
    ...
    { name: 'Expired', path: '/owner/dues?tab=expired', icon: <Clock size={20} /> },
  ...
  const isLinkActive = (link) => {
    ...
    const linkSearch = link.path.split('?')[1] || '';
    const currentPath = location.pathname;
    const currentSearch = location.search.substring(1);

    if (linkPath === '/owner' && currentPath === '/owner/dashboard') {
      return true;
    }
    ...
    if (linkSearch) {
      ...
      if (linkPath === '/owner/dues') {
        return !currentSearch.includes('tab=expired');
      }
      return true;
    }
  };
        ...
        {navLinks.map((link) => (
          <NavLink
            ...
            className={() => {
              ...
              return `flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 group ${active
